...
namespace AspNet.Identity.SQL
{
        ...
        where TUser : IdentityUser
    {
        ...
        public Task RemoveFromRoleAsync(TUser user, string role)
        {
            ...
            userRolesTable.DeleteByRoleName(user.Id, role);
        ...
        public Task<List<TUser>> GetUsers(int pageIndex, int pageSize, string userName, string userRole, out int totalCount)
        {
            var userList = userTable.GetUsers(pageIndex, pageSize, userName, userRole, out totalCount) as List<TUser>;

            if (userList != null)
            {
                foreach (var item in userList)
                {
                    var roleList = userRolesTable.FindByUserId(item.Id);
                    if (roleList!=null && roleList.Count>0)
                    {
                        item.RoleName = string.Join(",",roleList);
                        item.RoleId = string.Join(",",roleTable.GetRoleId(roleList[0]));
                    }
