...
mapObject.ClearDealerToken = function () {
    for (var i = 0; i < mapObject.DealerToken.length; i++) {
        mapObject.DealerToken[i].setMap(null);
    }
}

mapObject.AddToken = function (y, x,dealerId ,titleName, addressName, content) {
    if (x == "" || y == "") {
        return;
    }
    ...
    mapObject.DealerToken.push(marker);
    marker.setMap(mapObject.Object);  //在地图上添加点
    AMap.event.addListener(marker, 'click', function () {
        mapObject.DealerAddress(x, y,dealerId, titleName, addressName);
...
mapObject.OpenWidow = function (title, content) {
    ...
    info.push(content);
    info.push("<br/><input type='button' value='选为经销商' onclick='mapObject.SetDealer()' />");
    info.push("</div></div>");
...
mapObject.addmarker = function (i, d) {
    ...
    var aa = function (e) {
        ...
        mapObject.AddressPoint(d.location.getLat(), d.location.getLng(), d.address);
    };
...
mapObject.keywordSearch_CallBack = function (data) {
    ...
    for (var i = 0; i < resultCount; i++) {
        ...
        resultStr += mapObject.TipContents(poiArr[i].type, poiArr[i].address, poiArr[i].tel, poiArr[i].location.getLat(), poiArr[i].location.getLng()) + "</td></tr></table></div>";
...
mapObject.TipContents = function (type, address, tel, x, y) {  //窗体内容
    ...
    str += "<br/> <label>设置为：</label>&nbsp;<input type='button' value='取车地点' onclick='mapObject.SetAddress(1)'/>&nbsp;<input type='button' value='送车地点' onclick='mapObject.SetAddress(2)' />";
    return str;
}
...
//经纬度距离
mapObject.DistanceLngLat = function (slng, slat, elng, elat) {
    var startLngLat = new AMap.LngLat(slng, slat);
    var endLngLat = new AMap.LngLat(elng, elat);
    return startLngLat.distance(endLngLat);
}


//外部回调方法
mapObject.AddressPoint = function (x, y, addressName, searchType) {
