var findPayPassword = angular.module('findPayPassword', []);

findPayPassword.controller('FindPayPasswordCtrl', function ($scope, $http, $routeParams, $location) {
    $scope.showId = "f1";
    $scope.formData = {};
    $scope.userInfo = {};
    ...
    $scope.load = function () {
        setTimeout(function () {
            ...
            var url = "/UserSecurity/GetLoginUserInfo";

            $http.post(url).success(function (largeLoad) {
                if (largeLoad.IsSuccess) {
                    $scope.userInfo = largeLoad.Data;
