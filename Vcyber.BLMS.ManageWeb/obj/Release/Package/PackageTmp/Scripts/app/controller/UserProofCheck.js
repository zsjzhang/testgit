...
userProofCheck.controller('userProofCheckCtrl', function ($scope, $http, $routeParams, $location) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, filter) {
        ...
        $http.get(url, { params: { phone: filter.phoneNumber, mlevel: filter.mLevelValue, paperwork: filter.mPaperWork, identitynumber: filter.identityNumber, status: filter.isProofed, StrCreateTime: filter.beginTime, StrEnd: filter.endTime, pageCount: pageSize, currentPage: (page - 1) * pageSize } }).success(function (largeLoad) {
