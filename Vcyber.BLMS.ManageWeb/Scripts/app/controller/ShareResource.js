        ...
        if (files && files.length) {
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                $upload.upload({
                    ...
                    if (type == "source") {
                        $("#uplimg").html('<em id="emlpad" style="display:inline-block; position:absolute; top:0; left:300px; width:32px; height:32px; line-height:32px;">' + progressPercentage + '%</em>');
                    }
...
shareRes.controller('ResDownloadCtrl', function ($scope, $http, $routeParams, $location) {
    ...
    $scope.downloadShareRes = function (row) {
        ...
        window.open("http://" + location.host + "/" + row.entity.LinkUrl);
        return null;
    };
