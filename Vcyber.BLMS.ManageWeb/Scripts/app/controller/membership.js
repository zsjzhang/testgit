...
membershipModule.controller('MembershipCtrl', function ($scope, $http, $routeParams) {
    ...
    $http.post('/Common/GetMembershipLevelJsonResult').success(function (data) {
        //data.Delete("索九会员");
        $scope.mLevelDw.data = data;
    });
    $http.post('/Common/GetMembershipStatusJsonResult').success(function (data) {
        $scope.statusDw.data = data;
    });
    //获取会员审批状态
    $http.post('/Common/GetMembershipApproveType').success(function (data) {
    ...
    $scope.search = function () {
        ...
        $scope.filterOptions.No = $scope.No;
    ...
    $scope.getPagedDataAsync = function (pageSize, page, filter) {
        setTimeout(function () {
            ...
            if ($scope.exactSearch) {
                ...
                $.post(url, { UserType: filter.UserType, PayNumber: filter.PayNumber, IsTmall: filter.IsTmall, Identity: filter.identity, NickName: filter.nickName, MLevel: filter.mLevelValue, Status: filter.statusValue, Skip: (page - 1) * pageSize, Count: pageSize, BeginTime: filter.beginTime, EndTime: filter.endTime, IDCard: filter.IDCard, DealerId: filter.DealerId, VIN: filter.VIN, PaperWork: filter.PaperWork, isComValue: filter.isComValue, AuthenticationTimeStart: filter.AuthenticationTimeStart, AuthenticationTimeEnd: filter.AuthenticationTimeEnd, BuyTimeStart: filter.BuyTimeStart, BuyTimeEnd: filter.BuyTimeEnd, "__RequestVerificationToken": document.getElementsByName('__RequestVerificationToken')[0].value, AuthenticationSource: filter.AuthenticationSource, CreatedPerson: filter.CreatedPerson, No: filter.No }).success(function (largeLoad) {
    ...
    $scope.gridOptions = {
            ...
            cellTemplate: '<a href="#/Membership/Detail/{{row.getProperty(\'Id\')}}">详情</a>  <a style="display:{{row.getProperty(\'Isadmin\')}}"  href="javascript:return void()" ng-show="row.entity.Status==1" style="color:red" ng-click="Frozen(row)">冻结</a> ' +
                ...
                ' <a style="display:{{row.getProperty(\'Isadmin\')}}" href="javascript:return void()" data-toggle="modal" data-target="#PhoneNumberModal" ng-click="updateINPage(row)">修改手机号</a>' +
                 ...
                 ' <a style="display:{{row.getProperty(\'Isadmin\')}}" href="javascript:return void()" data-toggle="modal" data-target="#SetWorthModal" ng-click="setInWorth(row)">设置财富</a>'+
                  ' <a style="display:{{row.getProperty(\'Isadmin\')}}"  href="javascript:return void() "data-toggle="modal" ng-show="row.entity.datastate==0" style="color:red" ng-click="FreezeUserintegral(row)">冻结积分</a>'
       ...
       , {
        ...
        , {
            ...
            sortable: false,
            width: 80,
            cellTemplate: '<div>{{ShowGender(row.getProperty(\'Gender\'))}}</div>'
        }, {
            field: 'City',
            displayName: '城市',
            sortable: false,
    ...
    $scope.FreezeUserintegral = function (row) {
        if (confirm("确定要冻结此用户积分？")) {
            $http.post('/Membership/FreezeUserintegral', { id: row.entity.Id }).success(function (data) {
                if (data.success) {
                    $scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage, $scope.filterOptions);
                    alert("操作成功！ \n");
                } else {
    ...
    $scope.updatePhoneNumberModal = function () {
        ...
        var postData = {};
        postData.id = $scope.selectItem.Id;
        postData.valideCode = $scope.PhoneValidateCode;
        postData.phoneNumber = $scope.selectItem.PhoneNumber;
        postData.__RequestVerificationToken = document.getElementsByName('__RequestVerificationToken')[0].value;
        $.ajax({
            type: "post",
            url: "/Membership/updatePhoneNumberModal",
            data: postData,
            success: function (data) {
                if (data.success) {
    ...
    $scope.updateIdentityNumberBy4S = function () {
        ...
        postData.valideCode = $scope.PhoneValidateCode;
        postData.phoneNumber = $scope.selectItem.PhoneNumber;
        postData.identityNumber = $scope.selectItem.IdentityNumber;
        $.ajax({
            type: "post",
            url: "/Membership/UpdateIdentityNumberBy4S",
            data: postData,
            success: function (data) {
                if (data.success) {
                    $scope.status = 3;
...
membershipModule.controller('MembershipCreateCtrl', function ($scope, $http, $routeParams) {
    ...
    $scope.create = function () {
        $scope.newMembership.IsCanClick = true;//隐藏
        // $scope.newMembership.NickName = $scope.nickName.NickName;
        // $scope.newMembership.__RequestVerificationToken = document.getElementsByName('__RequestVerificationToken')[0].value;
...
membershipModule.controller('MembershipScoreListCtrl', function ($scope, $http, $routeParams) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, filter) {
        ...
        setTimeout(function () {
            ...
            $http.get(eurl, { params: { userid: $routeParams.id, Skip: (page - 1) * pageSize, Count: pageSize * page } }).success(function (largeLoad) {
                $scope.setPagingData(largeLoad.data, page, pageSize, largeLoad.total_count);
            });
...
membershipModule.controller('MembershipApprovingListCtrl', function ($scope, $http, $routeParams) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, filter) {
        setTimeout(function () {
            ...
            $http.get(eurl, { params: { PayNumber: $scope.PayNumber, phoneNumber: $scope.phoneNumber, IdentityNumber: $scope.IdentityNumber, ApproveType: $scope.ApproveType, Status: $scope.Status, skip: (page - 1) * pageSize, count: pageSize, IsPay: $scope.IsPay, Amount: $scope.Amount, DealerId: $scope.DealerId, PaperWork: $scope.PaperWork, VINNumber: $scope.VINNumber, No: $scope.No } }).success(function (largeLoad) {
                $scope.setPagingData(largeLoad.data, page, pageSize, largeLoad.total_count);
            });
