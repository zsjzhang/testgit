...
afterSaleService.controller('AfterSaleServiceListCtrl', function ($scope, $http, $routeParams) {
    ...
    $scope.search = function () {
        ...
        //$scope.filterOptions.StarCreateTime = $("#StarCreateTime").val();
        //$scope.filterOptions.EndCreateTime = $("#EndCreateTime").val();  
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
        {
            ...
            sortable: false,
            width: 140,
            cellTemplate: "<span>{{ row.getProperty(col.field) | odditems }}</span>"
        }, {
            field: 'kqCreateTime',
            displayName: '发卡时间',
            sortable: false,
...
afterSaleService.filter('odditems', function () {
    return function (inputArray) {
        debugger;
        var d = new Date(Number(inputArray.replace("/Date(", "").replace(")/", "")));
...
afterSaleService.controller('AfterSaleRepairCtrl', function ($scope, $http, $routeParams) {
    ...
    $scope.SelectActivityTypes = [{ id: "全新悦动 悦纳迎新季", name: "全新悦动 悦纳迎新季"}];

    $scope.cardTypes = new Array();// 卡劵类型

   // $scope.SelectActivityTypes = new Array();
  //  $scope.SelectActivityType = "悦纳保养套餐券";
    $scope.load = function () {
        //var activityType = $("#selActivityType");
        //var option = $("<option>").text("悦纳保养套餐券").val("悦纳保养套餐券");
        //activityType.append(option);
    ...
    //核销
    $scope.submit = function () {
        if (!(/^1[3|4|5|7|8]\d{9}$/.test($scope.formData.PhoneNumber)) ) {
            alert("请输入正确的手机号");
            return false;
        }
      
        if (!$scope.isVINTrue) {
            alert("请先验证VIN码，再购买套餐");
            return false;
        }
        if (!$scope.IsMember) {
            alert("请先验证手机号");
        ...
        var url = "/CustomCard/AddUserCustCardRemel";
        var postData = {
            CustName: $scope.formData.CustName,
            OpenId: $scope.formData.OpenId,
            PhoneNumber: $scope.formData.PhoneNumber,
            VIN: $scope.formData.VIN,
            CardType: $scope.formData.SelectActivityType,
            Mileage: $scope.formData.Mileage,
            CarCategory: $scope.formData.CarCategory,
            BuyTime: $scope.formData.BuyDate
        };
        ...
        $http.post(url, postData).success(function (result) {
            if (result.IsSuccess) {
                ...
                $(".col-md-12").find("input[type='text']").val("");
                $scope.formData = {};
                $scope.formData.SelectActivityType = '全新悦动 悦纳迎新季';
                var oDate = new Date();
                $scope.formData.ConsumeDate = $scope.toDou(oDate.getFullYear()) + "-" + $scope.toDou(oDate.getMonth() + 1) + "-" + $scope.toDou(oDate.getDate());
    ...
    $scope.GetCustAndCarInfo = function () {
        console.log("nihao");
        var url = "/AfterSaleService/GetCustAndCarInfoBuy";
        var postData = {
            vin: $scope.formData.VIN,
            activity: $scope.formData.SelectCardType
        };
        ...
        $http.post(url, postData).success(function (result) {
            if (result.IsSuccess) {
                $scope.formData.CarCategory = result.Data.CarCategory;
                $scope.formData.BuyDate = result.Data.BuyTime;
                $scope.formData.UserId = result.Data.UserId;
                $scope.isVINTrue = true;
            } else {
                $scope.isVINTrue = false;
    ...
    $scope.ValidatePhoneCarInfo = function () {

        var reg = /^[1][3587][0-9]{9}$/;
        if (!reg.test($scope.formData.PhoneNumber)) {
            $scope.formData.PhoneNumber = "";
            alert("手机格式不正确");
            return false;
        }
        var url = "/AfterSaleService/IsMember";
        var postData = {
            PhoneNumber: $scope.formData.PhoneNumber
        };
        ...
        $http.post(url, postData).success(function (result) {
            ...
            if (result.date == "Y" && result.leve<10) {
                $scope.IsMember = false;
                alert("请先入会成为会员才有机会享受该套餐");
            }
            if (result.leve >= 10) {
                $scope.IsMember = true;
            }
            if (result.leve < 0) {
                $scope.IsMember = false;
                alert("请先入会成为会员才有机会享受该套餐");
                location.href = '/#/Membership/Create';
...
afterSaleService.controller('AfterSaleRepairSelectCtrl', function ($scope, $http, $routeParams) {
    ...
    $scope.cardTypes = new Array();// 卡劵类型
    $scope.SelectActivityTypes = [{ id: "全新悦动 悦纳迎新季", name: "全新悦动 悦纳迎新季" }];  //, id: "悦纳保养套餐券", name: "悦纳保养套餐券"
    var cardTypes38 = [{ activity: "三八活动", cardid: "pmXbSjvHWuRNfjXKz9PHnGSN4H6k", cardtitle: "4L机油兑换券" }
        , { activity: "三八活动", cardid: "pmXbSjl4yW3dzOJ6PlrsZx_ZOYFo", cardtitle: "4L机油8.5折兑换券" }
        , { activity: "三八活动", cardid: "pmXbSjlxGUgln7h2Fu0y4xBlK9kw", cardtitle: "基础保养体验券" }];
    ...
    $scope.load = function () {
        var activityType = $("#selActivityType");
        var url = "/CustomCard/GetSCServiceCardTypeList";
        //var option = $("<option>").text(actName).val(actName);
        //activityType.append(option);
        //$.ajax({
        //    url: url,
        //    type: 'Get',
    ...
    $scope.search = function () {
        ...
        $scope.filterOptions.EndCreateTime = $scope.formData.EndCreateTime;
        $scope.filterOptions.isactivity = $scope.formData.isactivity;
        $scope.filterOptions.CardCategory = $scope.formData.CardCategory;
        $scope.pagingOptions.currentPage = 1;
        $scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage, $scope.filterOptions);
    };
    ...
    $scope.getPagedDataAsync = function (pageSize, page, filter) {
        setTimeout(function () {
            var data;
            var url = "/AfterSaleService/SelectRepairList?";
            if (filter && filter.PhoneNumber != undefined) {
                url += "&PhoneNumber=" + filter.PhoneNumber;
            }
            if (filter && filter.CardType != undefined) {
            ...
            if (filter && filter.CardCategory != undefined) {
                url += "&CardCategory=" + filter.CardCategory;
            }
            //console.log(filter.isactivity);
            if (filter && filter.isactivity != undefined) {
                url += "&isactivity=" + filter.isactivity;
