...
namespace VcyBer.BLMS.MobileWeb
{
    ...
    public class SmsService : IIdentityMessageService
    {
        public Task SendAsync(IdentityMessage message)
        {
            SMSHelper.SendSMS(new SmsSendMessageDTO { Mobile = "18210201837", SmsContent = string.Format("您的验证码是： {0}， 请立即使用", "2342"), Type = SmsType.Captcha });
            return Task.FromResult(0);
        }
    }
    ...
    public class ApplicationUserManager : UserManager<ApplicationUser>
    {
        ...
        {
            var manager = new ApplicationUserManager(new FrontUserStore<ApplicationUser>(context.Get<ApplicationDbContext>() as SQLDatabase));
            // 配置用户名的验证逻辑
