...
OrderChange.BindCar = function () {
    ...
    $.ajax({
        type: "POST",
        url: common.resolveUrl("/OrderChange/BindIdentityNumber"),
        data: { userId: $("#hidUserId").val(), identityNumber: identityNumber },
        dataType: "json",
        success: function (data) {
            if (data.code == "200") {
                window.location.href = common.resolveUrl("/OrderChange/Inviter?source=" + $("#hidSource").val() + "&userId=" + $("#hidUserId").val() + "&flag=" + $("#hidFlag").val());
            }
            else {
